...
YarpCamera::YarpCamera(const std::string& port_prefix, const std::size_t& width, const double& height, const double& fx, const double& cx, const double& fy, const double& cy)
{
    /* Check YARP network. */
...
std::pair<bool, Transform<double, 3, Affine>> YarpCamera::get_pose(const bool& blocking)
{
    return std::make_pair(true, Transform<double, 3, Affine>::Identity());
}


std::pair<bool, cv::Mat> YarpCamera::get_rgb(const bool& blocking)
{
...
std::pair<bool, MatrixXf> YarpCamera::get_depth(const bool& blocking)
{
    // Get image

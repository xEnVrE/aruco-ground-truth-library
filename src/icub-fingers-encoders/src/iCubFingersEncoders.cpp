...
iCubFingersEncoders::iCubFingersEncoders(const std::string& robot_name, const std::string& laterality, const std::string& port_prefix, const std::string& context, const bool& use_analogs) :
    use_analogs_(use_analogs)
{
    ...
    /* Instantiate iCubFingers .*/
    fingers_["thumb"] = iCubFinger(laterality + "_thumb_b");
...
std::pair<bool, std::unordered_map<std::string, Eigen::VectorXd>> iCubFingersEncoders::get_encoders(const bool& blocking)
{
    /* Analog encoders. */
...
std::pair<bool, yarp::sig::Vector> iCubFingersEncoders::load_vector_double(const ResourceFinder& rf, const std::string key, const std::size_t size)
{
    bool ok = true;
